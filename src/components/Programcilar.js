...
export default function Programcilar() {
 ...
 const [öneÇıkan, setÖneÇıkan]=useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    return programcilar[öneÇıkan-1].isim
};

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: öneÇıkan==null?'royalblue':'gold',//null yazmasan da olabilir. // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim}{" "} <button onClick={() => {setÖneÇıkan(dev.id)/* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>

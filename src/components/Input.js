...
export default function Input() {
  ...
  const inputuDeğiştir = evt => {
    ...
    const { value } = evt.target;// const value =evt.target.value   demek oluyor.
  ...
  return (
    <div className='widget-input container'>
      ...
      <div id='output' style={stil}>
        {inputDegeri.toUpperCase()}</div> {/* ADIM 3 */}
      <div>
		<input id='input' type='text' value={inputDegeri} onChange={inputuDeğiştir} /> {/* ADIM 6 */}
